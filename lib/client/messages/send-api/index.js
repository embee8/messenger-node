 ...
 * @param {String} persona_id __Optional.__ The ID of the persona that should send this message.
 ...
 *   });
 */
function sendText (recipient, text, persona_id) {
  if (!text) {
    return Promise.reject('text required');
  }

  let payload = {'text': text};

  return this.callSendApi(recipient, payload, persona_id);
}
 ...
 *   });
 */
function sendQuickReplies (recipient, quick_replies, text, persona_id) {
  if (!quick_replies || !Array.isArray(quick_replies)) {
    return Promise.reject('quick_replies array required');
  }
 ...
 * @param {String} persona_id __Optional.__ The ID of the persona that should send this attachment.
 * @return {Promise<Object>}  The API response
 * @memberof  Client#
 ...
 *  });
 */
function sendAttachment (attachment, recipient, persona_id) {
  if (!attachment) {
    return Promise.reject('attachment object required');
  }

  let payload = new util.AttachmentPayload(attachment, recipient);
  if (!payload) {
    return Promise.reject('URL or file required');
  }
 ...
 * @param {String} persona_id __Optional.__ The ID of the persona that should send this template.
 * @return {Promise<Object>}  The API response
 * @memberof  Client#
 * @example <caption>Generic Template</caption> 
 * let recipient = {'id': '57024957309673'};
 ...
 *   });
 */
function sendTemplate (recipient, template, persona_id) {
  if (!template) {
    return Promise.reject('template object required');
  }
 ...
 * @param {String} persona_id __Optional.__ The ID of the persona that should send this sender action.
 * @return {Promise<Object>}  The API response
 * @memberof  Client#
 * @example
 * let recipient = {'id': '57024957309673'},
 ...
 *   });
 */
function sendSenderAction (recipient, sender_action, persona_id) {
  if (!sender_action) {
    return Promise.reject('sender_action required');
  }
...
/* API Request */
function callSendApi (recipient, payload, persona_id) {
  if (!recipient) {
    return Promise.reject('recipient object required');
  }

  if (!payload) {
    return Promise.reject('payload required');
  }
  ...
  if (payload.formData) {
    ...
    request_options.payload = new util.RequestPayload(recipient, payload, persona_id);
    if (!request_options.payload) return Promise.reject('error creating request payload');
  }
