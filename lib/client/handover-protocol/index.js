...
function passThreadControl (psid, target_app_id, metadata) {
  if (!target_app_id) {
    return Promise.reject('target_app_id required');
  } else if (target_app_id === 'page_inbox') {
    target_app_id = 263902037430900;
  }

  if (!psid) {
    return Promise.reject('PSID required to identify the thread to enact handover for');
  }
...
function getThreadOwner (psid) {
  if (!psid) {
    return Promise.reject('PSID required to identify the thread to get owner for');
  }
